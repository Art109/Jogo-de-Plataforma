...
public class World {
				...
				for(int yy = 0; yy < map.getHeight(); yy++){
					...
					if(pixelAtual == 0xFF000000) {
						...
						if(pixels[xx + ((yy-1) * map.getWidth())] == 0xFFFFFFFF && (yy - 1) >= 0 ){
							tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16,yy * 16,Game.spritesheet.getSprite(0,16,16,16));
						}
					}
					else if(pixelAtual == 0xFF221EFF) {
						//Jogador
						Game.player.setX(xx * 16);
						Game.player.setY(yy * 16);
					}
					else if(pixelAtual == 0xFF007C78) {
						tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16,yy * 16,Tile.TILE_WALL);
					}
					else if(pixelAtual == 0xFFFF0000) {
						Enemy enemy = new Enemy(xx * 16,yy *16,16,16,1);
	...
	public static void restartGame(){
		Game.player = new Player(Game.WIDTH/3,Game.HEIGHT/2,16,16,2);
		Game.entities.clear();
